...
This is the new application entry point that integrates:
    ...
    - ``AIAccessManagerV2`` — uses RuntimeRegistry + QueryAdapter
...
async def lifespan(app: FastAPI) -> AsyncGenerator[None, None]:
    ...
    # Initialize AIAccessManagerV2
    state.ai_manager = AIAccessManagerV2(
        runtime_registry=runtime_registry,
        query_adapters=query_adapters,
        event_bus=state.event_bus,
    )
    logger.info("AIAccessManagerV2 initialized")
    ...
    if state.ai_manager:
        pass  # AIAccessManagerV2 doesn't have destroy()
    TraceStore.reset()
